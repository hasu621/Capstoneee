...
            
            ...
            if (parsedUser.verification_status !== 'Verified') {
                ...
                navigate(`/register/${parsedUser.role}?s=${parsedUser.verification_status.toLowerCase()}`); 
                setLoading(false); 
...
            if (parsedUser.role && parsedUser.role.toLowerCase() !== 'faculty' && parsedUser.role.toLowerCase() !== 'dept_head') {
                navigate('/'); 
                setLoading(false);
                return;
            }
...
            });
            setLoading(false); // End loading after successful data load

        } else {
            navigate('/');
            setLoading(false); // End loading if no user found
        }
    }, [navigate]);
